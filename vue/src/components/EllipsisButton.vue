...
<div id="ellipsis-button" >
        <font-awesome-icon icon="ellipsis-h" @click="buttonClicked" v-click-outside="hideContextMenu"/>
        <ki-context 
            ref="kiContext"
            minWidth='1em'
            maxWidth='15em'
      ...
      let items = [
        {
          ...
          click: () => {
            if (confirm('Are you sure you want to update this post?')) {
              this.$router.push({path: `/update/${this.postId}`})
            }
